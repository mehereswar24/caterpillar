    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] text-gray-500 font-mono">Real-time Safety Telemetry</span>
        </div>

        {activeAlerts.length === 0 ? (
          <div className="bg-[#121212] border border-[#222] rounded-2xl p-6 text-center text-emerald-400 text-xs flex items-center justify-center gap-2">
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className="text-xs font-bold text-white uppercase">{alert.title}</span>
                        <span className="text-[10px] text-gray-500 font-mono">({alert.time})</span>
                      </div>
                      <p className="text-xs text-gray-200 mt-1">{alert.message}</p>
                  ...
                  <button
                    ...
                    className="self-end sm:self-center bg-[#222] hover:bg-[#333] text-gray-200 text-xs font-bold px-3 py-1.5 rounded-xl border border-[#444] transition flex items-center gap-1.5 flex-shrink-0"
                  >
      ...
      <div className="space-y-3 pt-4 border-t border-[#222]">
        ...
        <div className="bg-[#111111] border border-[#242424] rounded-2xl divide-y divide-[#1e1e1e]">
          {resolvedAlerts.map(r => (
            <div key={r.id} className="p-3.5 flex items-center justify-between gap-4 text-xs font-mono">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-gray-200 font-sans font-bold mr-2">{r.title}</span>
                  <span className="text-gray-400 font-sans text-[11px]">{r.message}</span>
                </div>
              </div>
