...
// Telemetry presets matching sim scenes
const SCENE_TELEMETRY = {
...
// Animated SVG excavator — fully drawn in SVG
function ExcavatorSVG({ tilt, armAngle, bucketAngle, alerts, scene }) {
  ...
  // Tilt transform on whole machine
  const tiltRad = (tilt - 2) * 0.8; // visual exaggeration

  return (
    <svg viewBox="0 0 420 260" className="w-full h-full" style={{ filter: hasAlert ? 'drop-shadow(0 0 12px rgba(239,68,68,0.5))' : 'none' }}>
      {/* Ground */}
      ...
      <g transform={`translate(200,210) rotate(${tiltRad}) translate(-200,-210)`}>
        ...
        <g transform={`rotate(${armAngle}, 215, 145)`}>
          <rect x="110" y="125" width="115" height="16" rx="6" fill="#facc15" stroke="#ca8a04" strokeWidth="1.5"
            transform="rotate(-35 110 125)"/>
          {/* Hydraulic cylinder on boom */}
          <rect x="120" y="128" width="70" height="8" rx="3" fill="#d97706" opacity="0.8"
            transform="rotate(-35 110 125)"/>

          {/* STICK */}
          <g transform={`rotate(${bucketAngle}, 175, 85)`}>
            <rect x="155" y="68" width="90" height="13" rx="5" fill="#facc15" stroke="#ca8a04" strokeWidth="1.5"
              transform="rotate(15 155 68)"/>
            {/* Bucket */}
            <g transform="translate(232, 72)">
              ...
              {/* Bucket teeth */}
              {[0,1,2,3].map(i => (
                <rect key={i} x={2 + i*7} y={25} width={4} height={7} rx={1} fill="#44403c"/>
              ))}
            </g>
          </g>
        </g>
        ...
        {/* === WORKER PROXIMITY DOTS === */}
        {alerts.find(a => a.type.includes('WORKER')) && [
          {x: 50, y: 165}, {x: 370, y: 170}
        ].map((pos, i) => (
          <g key={i}>
            <circle cx={pos.x} cy={pos.y} r={12} fill="#ef4444" opacity={0.2}>
              <animate attributeName="r" values="12;20;12" dur="1s" repeatCount="indefinite"/>
            </circle>
            <text x={pos.x - 5} y={pos.y + 5} fontSize="14">👷</text>
          </g>
        ))}
        ...
        {tilt > 5 && (
          <g>
            <line x1="320" y1="145" x2={320 + Math.sin(tiltRad * Math.PI / 180) * 30} y2={145 - Math.cos(tiltRad * Math.PI / 180) * 30}
      ...
      {/* Alert flash overlay on critical */}
      {critical && (
...
export default function TruckSimulator() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setTelemetry(noisy);
      setAlerts(getAlerts(noisy, scene));
    }, 800);
    return () => clearInterval(interval);
  }, [scene]);

  // Hit anomaly API on scene change
  ...
  return (
    <div className="bg-gray-900 rounded-2xl border border-gray-700 overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-0">
        {/* Excavator canvas */}
        <div className="lg:col-span-2 relative bg-gray-950 p-4" style={{ minHeight: 280 }}>
          <ExcavatorSVG
            tilt={telemetry.tilt}
            armAngle={armAngle}
          ...
          {/* Alert overlays on top of machine */}
          <div className="absolute top-3 left-3 space-y-1.5 max-w-xs">
            {alerts.map((a, i) => (
              <div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold backdrop-blur-sm"
                ...
                <span>{a.icon}</span>
                <span>{a.type}</span>
                {['#ef4444','#f43f5e'].includes(a.color) && (
        ...
        {/* Right panel — gauges + scene selector */}
        <div className="border-l border-gray-700 p-4 flex flex-col gap-3">
          {/* Gauges */}
          <Gauge label="Engine RPM"    value={telemetry.rpm}       min={600}  max={2500} unit="rpm" warn={1900} danger={2100} icon="⚙️"/>
          <Gauge label="Hydraulic"     value={telemetry.hydraulic} min={100}  max={320}  unit="bar" warn={250}  danger={280}  icon="💧"/>
          <Gauge label="Eng. Temp"     value={telemetry.temp}      min={30}   max={110}  unit="°C"  warn={90}   danger={100}  icon="🌡"/>
          <Gauge label="Fuel Level"    value={telemetry.fuel}      min={0}    max={100}  unit="%"   warn={20}   danger={10}   icon="⛽"/>
          <Gauge label="Tilt Angle"    value={telemetry.tilt}      min={0}    max={25}   unit="°"   warn={12}   danger={15}   icon="📐"/>
          <Gauge label="Speed"         value={telemetry.speed}     min={0}    max={20}   unit="kph" warn={10}   danger={15}   icon="💨"/>

          {/* Seatbelt + workers */}
          <div className="grid grid-cols-2 gap-2">
            <div className={`rounded-lg p-2 text-center text-xs font-bold border ${telemetry.seatbelt ? 'bg-green-900/30 border-green-700 text-green-400' : 'bg-red-900/30 border-red-600 text-red-400'}`}>
              🪑 {telemetry.seatbelt ? 'Fastened' : 'UNFASTENED'}
            </div>
            <div className={`rounded-lg p-2 text-center text-xs font-bold border ${telemetry.workers === 0 ? 'bg-green-900/30 border-green-700 text-green-400' : 'bg-red-900/30 border-red-600 text-red-400'}`}>
              👷 {telemetry.workers === 0 ? 'Zone Clear' : `${telemetry.workers} IN ZONE`}
            </div>
          </div>

          {/* Scene selector */}
          <div className="mt-auto">
            <div className="text-gray-500 text-xs mb-2">INJECT SCENARIO</div>
            <div className="grid grid-cols-2 gap-1.5">
              {SCENES.map(s => (
                <button key={s.id} onClick={() => loadScene(s.id)}
                  className="text-xs py-1.5 px-2 rounded font-medium border transition-all"
                  style={{
